...
class DemucsSeparatorService:
    ...
    def process(self, uploaded_file):
        ...
        stem_name = f'{Path(safe_name).stem}_{unique_id}{suffix}'
        upload_relative_path = f'uploads/{stem_name}'
        ...
        with default_storage.open(upload_relative_path, 'wb+') as destination:
        ...
        return SeparationResult(
            ...
            upload_relative_path=f'media/{upload_relative_path}'.replace('\', '/'),
    ...
    def _find_vocals_file(self, result_dir: Path) -> Path | None:
        ...
        for path in candidates:
            if path.is_file() and 'vocal' in path.stem.lower():
                return path
        return None

    def _find_instrumental_file(self, result_dir: Path, vocals_file: Path | None) -> Path | None:
        candidates = [p for p in result_dir.rglob('*') if p.is_file() and p.suffix.lower() in {'.wav', '.mp3'}]
        ...
        remaining = [p for p in candidates if vocals_file is None or p.resolve() != vocals_file.resolve()]
        if remaining:
            return remaining[0]
        return None
    ...
    def _relative_media_path(self, absolute_path: Path) -> str:
        ...
        return f'media/{relative}'.replace('\', '/')
